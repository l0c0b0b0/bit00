...
class NaabuTCPTop1000:
    ...
    async def run(target, tag, output, module):
        ...
        """Run nmap scan."""
        cmd = f"/usr/bin/naabu -host {target} -silent -no-color -c 10 -rate 1000 --top-ports 1000 -nmap-cli '-vv -Pn --min-rate=1000 -T4 --open -O --osscan-guess --max-os-tries 5 -sV --traceroute --disable-arp-ping --source-port 53 \
